...
if(get('SPOTNET_SERVER_HOST' , False) != ""):
    SPOTNET_SERVER_HOST = get('SPOTNET_SERVER_HOST' , False)
else :
    raise ImproperlyConfigured("You haven't set the mandatory setting SPOTNET_SERVER_HOST.")
...
SPOTNET_UPDATE_MAXPOST         = get(  'SPOTNET_UPDATE_MAXPOST',         50000)
...
def load():
    ...
    global ART
    global ICON      
    global ROOT_PATH  #           = Core.storage.join_path(Core.storage.join_path(Core.app_support_path, Core.config.bundles_dir_name) , 'Plex-spotnet-python.bundle' , 'Contents')
    global IMAGE_DIR    #             = Core.storage.join_path(Core.storage.join_path(ROOT_PATH , 'Resources' , '%s.jpg'))
    global DB_DIR    #            = Core.storage.join_path(Core.storage.join_path(ROOT_PATH , 'Database'))

    global QUEUE  #               = {}
   
    global SPOTNET_SERVER_PORT   #  = get(  'SPOTNET_SERVER_PORT',            119)
    global SPOTNET_SERVER_SSL  #   = get(  'SPOTNET_SERVER_SSL',             False)
    global SERVER_USERNAME      #  = get(  'SPOTNET_SERVER_USERNAME',        None)
    global SERVER_PASSWORD     #   = get(  'SPOTNET_SERVER_PASSWORD',        None)
    global SERVER_READERMODE   #   = get(  'SPOTNET_SERVER_READERMODE',      False)

    global SPOTNET_UPDATE_MAXPOST   #       = get(  'SPOTNET_UPDATE_MAXPOST',         50000)
    global SPOTNET_UPDATE_EXTRA  #         = get(  'SPOTNET_UPDATE_EXTRA',           15000)
    global SPOTNET_UPDATE_BULK_COUNT  #     = get(  'SPOTNET_UPDATE_BULK_COUNT',      1000)
    global SPOTNET_UPDATE_GROUPS    #       = get(  'SPOTNET_UPDATE_GROUPS',          ('free.pt', 'spot.net', ))
    global UPDATE_COMMENT_GROUPS   #       = get(  'SPOTNET_UPDATE_COMMENT_GROUPS',  ('free.usenet', ))
    global UPDATE_DISCARD_ENDINGS    #      = get(  'SPOTNET_UPDATE_DISCARD_ENDINGS', (SPOTNET_SERVER_HOST,))

    global MAX_CONNECTIONS  #      = int(get("MAX_CONNECTIONS" , 3))    
    global MAX_AGE         #       = get(  'MAX_AGE' ,                       31)
    global UPDATE_ON_BOOT  #        = get(  'SPOTNET_UPDATE_ON_BOOT',         False)
    global CLEANUP_MINAGE  #        = get(  'SPOTNET_CLEANUP_MINAGE',         1999)
    global POST_PER_PAGE  #        = get(  'SPOTNET_POST_PER_PAGE',          30)
    global KEEP_PRON  #
    ...
    ROOT_PATH              = Core.storage.join_path(Core.storage.join_path(Core.app_support_path, Core.config.bundles_dir_name) , 'Plex-spotnet-python.bundle' , 'Contents')
    IMAGE_DIR                = Core.storage.join_path(Core.storage.join_path(ROOT_PATH , 'Resources' , '%s.jpg'))
    DB_DIR                 = Core.storage.join_path(Core.storage.join_path(ROOT_PATH , 'Database'))
