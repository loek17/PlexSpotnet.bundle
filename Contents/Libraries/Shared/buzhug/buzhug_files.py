...
class ObjectFile(StringFile):
    ...
    def to_block(self,value):
        ...
        try:
            ...
            raise ValueError,'Bad type : expected pickble object, got %s %s' %(value , value.__class__)
        else:
            return StringFile.to_block(self ,str)
    
    def from_block(self,block):
        if block == '!\n':
            return None
        else:
            return pickle.loads(StringFile.from_block(self,block))

class ListFile(ObjectFile):
    """ list objects """

    def to_block(self , value):
        return ObjectFile.to_block(self , value)

    def from_block(self,block):
        return ObjectFile.from_block(self , block)

class DictFile(ObjectFile):
    """ dict objects """
